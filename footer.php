<footer class="mdl-mega-footer">
            <div class="mdl-mega-footer--middle-section">
                <div class="mdl-mega-footer--drop-down-section">
                    <input class="mdl-mega-footer--heading-checkbox" type="checkbox" checked>
                    <h1 class="mdl-mega-footer--heading">Info</h1>
                    <ul class="mdl-mega-footer--link-list">
                        <li><a href="https://docs.parrotsec.org/what-is-parrot">Acerca de</a></li>
                        <li><a href="https://docs.parrotsec.org/terms-and-conditions">Términos</a></li>
                        <li><a href="https://docs.parrotsec.org/partners">Partners</a></li>
                        <li><a href="https://blog.parrotsec.org">Noticias</a></li>
                    </ul>
                </div>
                <div class="mdl-mega-footer--drop-down-section">
                    <input class="mdl-mega-footer--heading-checkbox" type="checkbox" checked>
                    <h1 class="mdl-mega-footer--heading">¿Qué se dice de nosotros?</h1>
                    <ul class="mdl-mega-footer--link-list">
                        <li><a target="_blank" href="https://en.wikipedia.org/wiki/Parrot_Security_OS">Wikipedia</a></li>
                        <li><a target="_blank" href="http://distrowatch.com/table.php?distribution=parrotsecurity">Distrowatch</a></li>
                        <li><a target="_blank" href="http://linux.softpedia.com/get/System/Operating-Systems/Linux-Distributions/Parrot-security-OS-102936.shtml">Softpedia</a></li>
                    </ul>
                </div>
                <div class="mdl-mega-footer--drop-down-section">
                    <input class="mdl-mega-footer--heading-checkbox" type="checkbox" checked>
                    <h1 class="mdl-mega-footer--heading">Comunidad</h1>
                    <ul class="mdl-mega-footer--link-list">
                        <li><a target="_blank" href="https://t.me/ParrotSpanishGroup">Telegram (Grupo en español)</a></li>
                        <li><a target="_blank" href="http://lists.parrotsec.org/listinfo">Lista de correo</a></li>
                        <li><a target="_blank" href="https://www.facebook.com/parrot.es/">FanPage en Facebook</a></li>
                    </ul>
                </div>
                <div class="mdl-mega-footer--drop-down-section">
                    <input class="mdl-mega-footer--heading-checkbox" type="checkbox" checked>
                    <h1 class="mdl-mega-footer--heading">Social Pages</h1>
                    <ul class="mdl-mega-footer--link-list">
                        <li><a target="_blank" href="https://github.com/josegatica/parrotsec-es">GitHub</a></li>
                        <li><a target="_blank" href="https://www.facebook.com/parrot.es/">Facebook</a></li>
                        <li><a target="_blank" href="https://twitter.com/ParrotSec_es">Twitter</a></li>
                        <li><a target="_blank" href="https://www.youtube.com/channel/UCj2dezzTc_Oy9eAEwBBodpw">Youtube</a></li>
                    </ul>
                </div>
            </div>
            <div class="mdl-mega-footer--bottom-section">
                <div class="mdl-logo">
                    Notas legales
                </div>
                Parrot Security OS 2011-<?php echo date(Y,time()) ?> &copy; <a href="https://docs.parrotsec.org/user/palinuro">Lorenzo Faletra</a> - <a href="https://www.gnu.org/licenses/gpl-3.0.en.html" target="_blank">GNU/GPL v3</a><br>
                <b>Orgullosamente desarrollado por Lorenzo Faletra y la comunidad de habla hispana de ParrotSec</b>
            </div>
</footer>
